...
A polished, responsive analytics dashboard with accessible light and high-contrast dark themes, built as an installable Progressive Web App. Northstar presents revenue, customer, conversion, acquisition, and transaction signals using a deterministic local dataset, so every demo is fast and repeatable without a backend.
...
- **Persistent accessible themes** follow the system preference on first visit and remember a user's explicit light/dark selection.
